  ...
  try {
    ...
    for (const d of data) {
      ...
      const line: CartLine = {
        ...
        unitPrice: d.unitPrice ?? product.price,
        variationKey: d.variationKey ?? `${product.id}:base`,
      };

      // Only add variationLabel when it actually exists
      if (d.variationLabel !== undefined) {
        line.variationLabel = d.variationLabel;
      }
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const add = useCallback(
    (
      ...
      variationLabel?: string,
      unitPrice = p.price,
      variationKey = `${p.id}:base`
    ) => {
      setLines((prev) => {
        const existing = prev.find(
          (l) => l.variationKey === variationKey
        );
        ...
        if (variationLabel !== undefined) {
          newLine.variationLabel = variationLabel;
        }
  ...
  const setQty = useCallback(
    (variationKey: string, qty: number) => {
      setLines((prev) =>
        qty <= 0
          ? prev.filter((l) => l.variationKey !== variationKey)
          : prev.map((l) =>
              l.variationKey === variationKey
                ? { ...l, qty }
                : l
